...
class AE(nn.Module):
    def __init__(self, num_in_ch, features_root, shrink_thres, 
        ...
        for i in range(level):
            self.down_blocks.append(down(self.filter_list[i], self.filter_list[i+1], use_gam=False))   #￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥
            if ops[i] == 'concat':
                filter = self.filter_list[level-i] + self.filter_list[level-i-1]#//2
            else:
                filter = self.filter_list[level-i]
            self.up_blocks.append(up(filter, self.filter_list[level-1-i], op=ops[i], use_gam=False))
        ...
        if dist:
            ...
            for i in range(level):
                if ops[i] == 'concat':
                    ...
                    filter = self.filter_list[level-i]
                self.teacher_ups.append(up(filter, self.filter_list[level-1-i], op=ops[i], use_gam=False))
